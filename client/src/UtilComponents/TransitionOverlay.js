...
class TransitionOverlay extends Component {
  componentDidMount() {
    if (this.props.action === 'PUSH') {
      TweenMax.staggerFromTo([this.whiteOverlay, this.blueOverlay, this.blackOverlay], 1.8, {x: "-120%"}, {x: "150%"}, 0.3)
    }
    else {
      TweenMax.staggerFromTo([this.blackOverlay, this.blueOverlay, this.whiteOverlay], 1.8, {x: "120%"}, {x: "-150%"}, 0.3)
    }
  }
